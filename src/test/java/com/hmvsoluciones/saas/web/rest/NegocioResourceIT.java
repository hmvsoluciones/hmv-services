...
class NegocioResourceIT {
    ...
    void getAllNegociosByResponsableIsInShouldWork() throws Exception {
        ...
        defaultNegocioFiltering(
            "responsable.in=" + DEFAULT_RESPONSABLE + "," + UPDATED_RESPONSABLE,
            "responsable.in=" + UPDATED_RESPONSABLE
        );
    }
    ...
    void getAllNegociosBySubscriptionKeyContainsSomething() throws Exception {
        ...
        defaultNegocioFiltering(
            "subscriptionKey.contains=" + DEFAULT_SUBSCRIPTION_KEY,
            "subscriptionKey.contains=" + UPDATED_SUBSCRIPTION_KEY
        );
    }
    ...
    void putExistingNegocio() throws Exception {
        ...
        restNegocioMockMvc
            .perform(
                put(ENTITY_API_URL_ID, negocioDTO.getId()).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(negocioDTO))
            )
            .andExpect(status().isOk());
    ...
    void patchNonExistingNegocio() throws Exception {
        ...
        restNegocioMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, negocioDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(negocioDTO))
            )
            .andExpect(status().isBadRequest());
