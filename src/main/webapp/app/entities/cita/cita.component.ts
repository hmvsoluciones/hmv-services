  ...
  setup() {
    ...
    const retrieveCitas = async () => {
      ...
      try {
        ...
        const res = await citaService().retrieve(paginationQuery);
    ...
    const prepareRemove = (instance: ICita) => {
    ...
    const removeCita = async () => {
      try {
        await citaService().delete(removeId.value);
        const message = `A Cita is deleted with identifier ${removeId.value}`;
